...
namespace fIT.App.Utilities.Navigation
{
    public class NavigationFrame : IViewModelNavigation
    {
        ...
        public async Task<ViewModelBase> PopAsPopUpAsync(bool animate = true)
        {
            await PopupNavigation.PopAsync(animate);
            return this.CurrentViewModel;
        }

        public async Task PushAsPopUpAsync(ViewModelBase viewModel, bool animate = true)
        {
            var page = CreatePageForViewModel(viewModel) as PopupPage;
            if (page == null)
            {
                throw new ArgumentException("The Page associated with the ViewModel must inherit from PopupPage");
            }
